...
export default function ProofOfIdentificationForm(): JSX.Element {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      submitData.append('name', user.firstName + ' ' + user.lastName);
  ...
  return (
    <div className="p-6 rounded-lg max-w-5xl mx-auto dark:bg-emerald-950/10 dark:text-white bg-white text-gray-900 border border-gray-100 dark:border-emerald-800/30 backdrop-blur-sm">
      ...
      <div className="mb-6 w-full text-sm">
        <p className="mb-2 dark:text-gray-300 text-gray-700 max-w-[590px]">
          To verify your identification, please upload one of the following
          proof of identification documents: ID Card, passport or driver's
          license. All sides containing your personal details are required.
        </p>
        <p className="dark:text-gray-300 text-gray-700 max-w-[620px]">
          The documents must include: full name, ID number, photograph, date of
          birth, place of birth, nationality
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            >
              ...
              >
                <Camera
                  size={32}
                  className="mb-2 dark:text-gray-400 text-gray-500"
                />
                {formData.frontImage ? (
            ...
            {errors.frontImage && (
              <p className="mt-1 text-red-500 text-sm">{errors.frontImage}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.backImage && (
              <p className="mt-1 text-red-500 text-sm">{errors.backImage}</p>
            )}
          </div>
        </div>
